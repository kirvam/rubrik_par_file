...
# config
my $fields = 14; #there must be this many fields;
...
'Total Archive Storage (GB)',
'Local Storage Growth (MB/day)',
'Archive Storage Growth (MB/day)'
);
...
foreach my $key ( sort ( keys %$ref ) ){
  ...
  if($key =~ m/global/gi){ print "found Owner: $key.\n"; next; };
...
###    push @array, $line [$ii];
   ...
   my $owner = $array[5];
   my @arr = ( $array[5], $array[0], $array[4], $array[11] );
   if(${$refHoA}{$owner}){
#          push @{ $refHoA->{$owner} }, [ @array ];
           push @{ $refHoA->{$owner} }, [ @arr ];
      }  
    else {
#       @{ $refHoA->{$owner} } = [ @array ];
        @{ $refHoA->{$owner} } = [ @arr ];
